...
def main():
    ...
    parser.add_argument('-conf', type=str, help='Path to opt YAML file.', default='./options/train/train_in_sdsd.yml')
    parser.add_argument('--launcher', choices=['none', 'pytorch'], default='none',
                        help='job launcher')
    parser.add_argument('--local_rank', type=int, default=0)
    args = parser.parse_args()

    conf = opt.parse(args.conf, is_train=True)
    ...
    if conf['path'].get('resume_state', None):
        # distributed resuming: all load into default GPU
        device_id = torch.cuda.current_device()
        resume_state = torch.load(conf['path']['resume_state'],
                                  map_location=lambda storage, loc: storage.cuda(device_id))
        opt.check_resume(conf, resume_state['iter'])  # check resume options
    else:
        resume_state = None
    ...
    if rank <= 0:  # normal training (rank -1) OR distributed training (rank 0)
        if resume_state is None:
            util.mkdir_and_rename(
                conf['path']['experiments_root'])  # rename experiment folder if exists
            util.mkdirs((path for key, path in conf['path'].items() if not key == 'experiments_root'
                         and 'pretrain_model' not in key and 'resume' not in key))

        # opt loggers. Before it, the log will not work
        util.setup_logger('base', conf['path']['log'], 'train_' + conf['name'], level=logging.INFO,
                          screen=True, tofile=True)
        logger = logging.getLogger('base')
        logger.info(opt.dict2str(conf))
        # tensorboard logger
        if conf['use_tb_logger'] and 'debug' not in conf['name']:
            ...
            tb_logger = SummaryWriter(log_dir='./tb_logger/' + conf['name'])
    else:
        util.setup_logger('base', conf['path']['log'], 'train', level=logging.INFO, screen=True)
        logger = logging.getLogger('base')
    ...
    for phase, dataset_conf in conf['datasets'].items():
        if phase == 'train':
            train_set = create_dataset(dataset_conf)

            train_size = int(math.ceil(len(train_set) / dataset_conf['batch_size']))
            total_iters = int(conf['train']['niter'])
            ...
            if conf['dist']:
                ...
                train_sampler = None
            train_loader = create_dataloader(train_set, dataset_conf, conf, train_sampler)
            if rank <= 0:
    ...
    if resume_state:
        ...
        model.resume_training(resume_state)  # handle confimizers and schedulers
        del resume_state
    else:
        current_step = 0
        start_epoch = 0
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for _, train_data in enumerate(train_loader):
            ...
            model.update_learning_rate(current_step, warmup_iter=conf['train']['warmup_iter'])
            ...
            #### log
            if current_step % conf['logger']['print_freq'] == 0:
            ...
            #### validation
            if conf['datasets'].get('val', None) and current_step % conf['train']['val_freq'] == 0:
                if conf['dist']:
                    # multi-GPU testing
                    ...
                    for idx in range(rank, dist_val_set_len, world_size):
                        ...
                        if psnr_rlt.get(folder, None) is None:
                            psnr_rlt[folder] = torch.zeros(conf['datasets']['val']['N_frames'], dtype=torch.float32, device='cuda')
                        if ssim_rlt.get(folder, None) is None:
                            ssim_rlt[folder] = torch.zeros(conf['datasets']['val']['N_frames'], dtype=torch.float32, device='cuda')
            ...
            if current_step % conf['logger']['save_checkpoint_freq'] == 0:
                if rank <= 0:
