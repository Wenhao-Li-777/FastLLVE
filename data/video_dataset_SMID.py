...
class Video_Dataset_SMID(data.Dataset):
    def __init__(self, conf):
        super(Video_Dataset_SMID, self).__init__()
        self.conf = conf
    ...
    def __getitem__(self, index):
        ...
        if self.conf['phase'] == 'train':
            ...
            rlt = util.augment_torch(img_LQ_l, self.conf['use_flip'], self.conf['use_rot'])
            img_LQ_l = rlt[0:self.conf['N_frames']]
            img_GT_l = rlt[self.conf['N_frames']:]
